"""Validate and repair the GeoNames->Wikidata QID assignments of a dataset.

This is the GeoCorpora validator. Its run over the 144-row GeoCorpora sample
produced the log committed at results/logs/fix_qids_geocorpora_144.log.

For every row it:
  1. checks whether the existing `qid` already matches the row's
     `location_name` (English label) and `country` (P17) in Wikidata; and,
  2. if not, searches Wikidata (wbsearchentities) for a candidate whose P17
     country matches the expected country, and rewrites `qid` to the correct
     item.

It writes a corrected CSV (in place by default) and prints a per-row log
identical in shape to results/logs/fix_qids_geocorpora_144.log.

Usage:
    python scripts/fix_qids.py [input-csv] [output-csv]

With no arguments it defaults to src/social_media_dataset.csv (in place).
No API key is required — the Wikidata SPARQL endpoint and search API are public.
"""
import csv
import sys
import time
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        input_path = PROJECT_ROOT / "src" / "social_media_dataset.csv"
    else:
        input_path = _resolve(sys.argv[1])

    output_path = input_path if len(sys.argv) < 3 else Path(sys.argv[2])
